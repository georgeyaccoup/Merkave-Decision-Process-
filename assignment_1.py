...
class GreedyAgent(main_agent.Agent):
    def agent_step(self, reward, observation=None):
        ...
        # 1) Update estimates for the action taken in the previous step
        last_action = self.last_action

        self.arm_count[last_action] += 1
        step_size = 1.0 / self.arm_count[last_action]
        self.q_values[last_action] += step_size * (reward - self.q_values[last_action])

        # 2) Choose the next greedy action (ties broken randomly)
...
class EpsilonGreedyAgent(main_agent.Agent):
    def agent_step(self, reward, observation):
        # 1) Update Q-values for the LAST action taken
        a = self.last_action

        # (Usually last_action is always valid here because agent_start picks the first action,
        # then agent_step is called with the reward from that action.)
        self.arm_count[a] += 1
        step_size = 1.0 / self.arm_count[a]          # incremental sample-average step size
        ...
        if np.random.random() < self.epsilon:
            ...
            best_actions = np.flatnonzero(self.q_values == max_q)
            current_action = np.random.choice(best_actions)
...
class EpsilonGreedyAgentConstantStepsize(main_agent.Agent):
    def agent_step(self, reward, observation):
        ...
        # 1) Update q_values for the LAST action using constant step size
        a = self.last_action
        self.arm_count[a] += 1  # still useful for logging/analysis, even if not used in the update
        self.q_values[a] += self.step_size * (reward - self.q_values[a])

        # 2) Choose action using epsilon-greedy (same as before)
        if np.random.random() < self.epsilon:
            current_action = np.random.randint(len(self.q_values))
        else:
            max_q = np.max(self.q_values)
